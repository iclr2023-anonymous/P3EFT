...
class ModelWithDistCorReg(torch.nn.Module):
    def __init__(self, base_model, seed=0, device='cuda:0',
                 lora_rank=8, lora_alpha=8, lora_dropout=0.0, regularizing_weight=0.01,
        ...
        #         self.deberta.requires_grad_(True) #without this gradient checkpointing wont work
    ...
    def forward(self, input_ids, attention_mask, labels):
        ...
        if not self.model.training:
            with torch.random.fork_rng(
                ...
                baseline_activations = self.model(input_ids, attention_mask=attention_mask).logits
            
        change_active_adapter(self.model, 'baseline_lora')
        activations = self.model(input_ids, attention_mask=attention_mask).logits
        ...
        if self.regularizing_weight:
            regularizing_loss = compute_regularizing_loss(activations, labels)
            loss += self.regularizing_weight * regularizing_loss
            return (loss, logits, [cur_acts for cur_acts in [activations]], 
                [shifts], activations, regularizing_loss)
        return (loss, logits, [cur_acts for cur_acts in [activations]], 
                [shifts], activations)
