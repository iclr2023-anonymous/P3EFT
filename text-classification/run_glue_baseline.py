...
def compute_regularizing_loss(activations, labels):
...
def compute_custom_metrics(p: EvalPrediction):
    ...
    for cur_lora_activations in p.predictions[1]:
        activations_kmeans.append(
            compute_kmeans_acc(cur_lora_activations, p.label_ids)
        )
    ...
    for cur_lora_shifts in p.predictions[2]:
        shifts_kmeans.append(compute_kmeans_acc(cur_lora_shifts, p.label_ids))
    ...
    custom_metrics_dict = {
        ...
        "reg_loss": np.mean(p.predictions[-1])
    }
    return custom_metrics_dict

            
class ModelWithDistCorReg(torch.nn.Module):
    def __init__(self, base_model, seed=0, device='cuda:0',
                 lora_rank=8, lora_alpha=8, lora_dropout=0.0, 
                 regularizing_weight=0.01,
...
#         self.classifier = base_model.classifier
        ...
        self.head_hidden_dims.insert(0, self.model.config.pooler_hidden_size)
        for i in range(len(self.head_hidden_dims) - 1):
            self.classifier.add_module(f"linear{i}", nn.Linear(self.head_hidden_dims[i], self.head_hidden_dims[i+1]))
            self.classifier.add_module(f"relu{i}", nn.ReLU())
        self.classifier.add_module("linear_final", nn.Linear(self.head_hidden_dims[-1], 2))
        print(self.classifier)
        self.num_labels = base_model.num_labels
        self.regularizing_weight = regularizing_weight
        
        self.neck_width = self.model.config.pooler_hidden_size
        self.device = device
        self.seed = seed

        peft_config = LoraConfig(peft_type=peft.PeftType.LORA, r=lora_rank,
                                     lora_alpha=lora_alpha, lora_dropout=lora_dropout)
        peft.get_peft_model(self.model, peft_config, adapter_name='baseline_lora');

    def forward(self, input_ids, attention_mask, labels):
        baseline_activations = 0
        if not self.model.training:
            with torch.random.fork_rng(
                ...
                baseline_activations = self.model(input_ids, attention_mask=attention_mask).last_hidden_state[:, 0]
            
        change_active_adapter(self.model, 'baseline_lora')
        activations = self.model(input_ids, attention_mask=attention_mask).last_hidden_state
        
        logits = self.classifier(self.pooler(activations))
        loss = F.cross_entropy(logits.view(-1, self.num_labels), labels.view(-1))
        
        with torch.no_grad():
            shifts = activations[:, 0] - baseline_activations
        
        if self.regularizing_weight:
            regularizing_loss = compute_regularizing_loss(activations[:, 0], labels)
            loss += self.regularizing_weight * regularizing_loss
            return (loss, logits, [cur_acts[:, 0] for cur_acts in [activations]], 
                    [shifts], activations[:, 0], regularizing_loss)
        return (loss, logits, [cur_acts[:, 0] for cur_acts in [activations]], 
                    [shifts], activations[:, 0])
...
class ModelArguments:
    ...
    regularizing_weight: Optional[float] = field(
        default=1.0,
        metadata={"help": "Disctance correlation regularizing loss weight."},
    )
    model_gradient_checkpointing: Optional[bool] = field(
        default=False,
        metadata={"help": "Will enable gradient checkpointing."},
    )
...
#         self.save_val_preds(p)
        result.update(compute_custom_metrics(p))
        return result
    ...
    model_with_dist_cor = ModelWithDistCorReg(
        base_model=model, seed=training_args.seed, device=training_args.device,
        lora_rank=model_args.lora_rank, lora_alpha=model_args.lora_alpha, 
        lora_dropout=model_args.lora_dropout, regularizing_weight=model_args.regularizing_weight,
        model_gradient_checkpointing=model_args.model_gradient_checkpointing
    )
    # Initialize our Trainer
    trainer = Trainer(
        model=model_with_dist_cor,
        args=training_args,
