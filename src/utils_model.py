...
def get_averaged_coefs(seed=0, n_of_loras=2, neck_width=1536):
    return torch.ones(n_of_loras) / n_of_loras  # .to(device, dtype=dtype)
...
class ModelWithMultipleLoras(torch.nn.Module):
    def __init__(
        ...
        self.coefs = (
            name2genfunc[self.method_name](
                seed=seed, n_of_loras=n_of_loras, neck_width=self.neck_width
            )
            * mult_std
        ).to(device=device)
        if self.n_of_loras:
    ...
    def forward(self, input_ids, attention_mask, labels):
        ...
        for i in range(max(self.n_of_loras, 1)):
            if self.n_of_loras:
                self.active_adapter = f"{self.method_name}_lora_{i}"
            else:
                self.active_adapter = " "
            with torch.random.fork_rng(
                ...
                if self.loras_gradient_checkpointing:
                    ...
                    change_active_adapter(self.model, self.active_adapter)
                    cur_lora_activations = self.model(
                        input_ids, attention_mask=attention_mask
                    ).last_hidden_state
        ...
        self.active_adapter = " "
        change_active_adapter(self.model, self.active_adapter)  # empty string = no adapter
        ...
        if self.activations_regularizing_weight:
            for cur_activations in different_loras_activations:
                ...
                logreg = get_fitted_logreg(
                    cur_activations.detach().cpu().numpy()[:, 0],
                    labels.cpu().numpy(),
                    seed=self.seed,
                )
                loss += (
        ...
        for cur_activations in different_loras_activations:
            cur_shift = cur_activations[:, 0] - baseline_activations
        ...
        return (
            loss,
            logits,
            [cur_acts[:, 0] for cur_acts in different_loras_activations],
            different_loras_shifts,
            activations[:, 0],
        )
...
def get_model_multiple_loras(base_model, model_args, training_args):
    ...
    print(
        sum(
            p.numel()
            for p in model_multiple_loras.pooler.parameters()
            if p.requires_grad
        )
        / 1e6
    )
